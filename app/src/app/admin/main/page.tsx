...
import { formSchemaRegister, FormValuesRegister } from "@/components/schema/schema-register";
...
export default function Page() {
    ...
    const onSubmit = async (values: FormValuesRegister) => {
        ...
        toast.success(`${success} registro(s) completados para ${positions.find(p => p.id === activeTab)?.name}`);
    ...
    return (
        <div className="w-full mx-auto p-4">
            <Card className="shadow-md p-4">
                <CardHeader>
                    <CardTitle className="text-2xl font-bold">Registro de Votos</CardTitle>
                </CardHeader>
                                ...
                                <TabsContent key={p.id} value={p.id} className="pt-4 space-y-4">
                                    <Form {...form}>
                                        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                                            <h3 className="font-semibold text-lg border-b pb-2">{p.name}</h3>
                                            ...
                                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                                {positionCandidates.map((c) => (
                                                    <Card key={c.id} className="p-3 border hover:shadow-lg transition-shadow">
                                                        ...
                                                        <FormField
                                                            ...
                                                            render={({ field }) => (
                                                                <FormItem className="mt-3">
                                                                    <FormLabel className="text-sm">Votos obtenidos</FormLabel>
                                                                    <FormControl>
                                                                        <Input
                                                                            type="number"
                                                                            min={0}
                                            ...
                                            >
                                                {isSubmitting ? "Registrando..." : `Registrar Votos para ${p.name}`}
                                            </Button>
                                        </form>
                                    </Form>
                                </TabsContent>
